...
- E2E 运行器已对检索/问答步骤增加有限瞬时重试（各最多 2 次，仅 5xx，重试次数记入报告 `transient_retries_used`，有副作用的上传/删除不重试）。动因（2026-08-01 下午实测）：宿主机到 `dashscope.aliyuncs.com` 的 TLS 握手间歇性失败（诊断服务器实测 12 次检索 2 次 500，服务端日志为 `start_tls` CONNECTION FAILED；同日 `api.github.com` 与 Docker Hub 出现同类超时，属宿主机网络环境问题而非代码回归——合并后 main 与此前两次通过的工作树逐字节比对仅差 PR #8 自身文件）。顺带的真实缺陷观察：查询路径的供应商调用没有任何服务端重试（索引路径有），单次瞬时网络故障即以 500 暴露给用户，可与相关度下限一并列入检索路径改进。
