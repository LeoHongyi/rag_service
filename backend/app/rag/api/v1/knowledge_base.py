...
from backend.app.rag.schema.knowledge_base import (
...
async def get_knowledge_base(
    db: CurrentSession, request: Request, pk: Annotated[int, Path(description='知识库 ID')]
) -> ResponseSchemaModel[GetKnowledgeBaseDetail]:
    user_id, is_admin = _identity(request)
    return response_base.success(
        data=await knowledge_base_service.get(db=db, pk=pk, user_id=user_id, is_admin=is_admin)
    )


@router.post('', summary='创建知识库', dependencies=[DependsJwtAuth])
async def create_knowledge_base(
    db: CurrentSessionTransaction, request: Request, obj: CreateKnowledgeBaseParam
) -> ResponseSchemaModel[GetKnowledgeBaseDetail]:
...
async def update_knowledge_base(
    ...
    user_id, is_admin = _identity(request)
    return response_base.success(
        data=await knowledge_base_service.update(db=db, pk=pk, user_id=user_id, is_admin=is_admin, obj=obj)
    )


@router.delete('/{pk}', summary='删除知识库', dependencies=[DependsJwtAuth])
async def delete_knowledge_base(
    db: CurrentSessionTransaction, request: Request, pk: Annotated[int, Path(description='知识库 ID')]
) -> ResponseModel:
    user_id, is_admin = _identity(request)
