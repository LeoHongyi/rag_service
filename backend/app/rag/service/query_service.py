...
class QueryService:
    ...
    async def retrieve(*, db: AsyncSession, user_id: int, is_admin: bool, obj: RetrieveParam) -> list[GetSourceDetail]:
        ids = await QueryService._accessible_kb_ids(db, user_id=user_id, is_admin=is_admin, requested=obj.knowledge_base_ids)
        vector = (await embedding_provider.embed([obj.question]))[0]
        base = (
            select(Chunk, Document.filename)
            .join(Document, Document.id == Chunk.document_id)
            .where(
                Chunk.knowledge_base_id.in_(ids), Chunk.deleted == 0,
                Document.status == DocumentStatus.READY, Document.deleted == 0,
            )
        )
        distance = Chunk.embedding.cosine_distance(vector).label('distance')
        dense_rows = (await db.execute(base.add_columns(distance).order_by(distance).limit(settings.RAG_DENSE_CANDIDATE_K))).all()
        ...
        for rank, (chunk, name, value) in enumerate(dense_rows, 1):
            merged[chunk.id] = {'chunk': chunk, 'name': name, 'score': 1 / (settings.RAG_RRF_K + rank), 'dense': max(0.0, 1 - float(value))}
        ...
        return [
            GetSourceDetail(
                knowledge_base_id=item['chunk'].knowledge_base_id, document_id=item['chunk'].document_id,
                document_name=str(item['name']), chunk_id=item['chunk'].id, chunk_index=item['chunk'].chunk_index,
                content=item['chunk'].content, score=round(float(item['score']), 6), citation=f'[S{index}]',
            )
    ...
    async def answer(*, db: AsyncSession, user_id: int, is_admin: bool, obj: AnswerParam) -> GetAnswerDetail:
        ...
        effective_mode = 'agentic' if obj.mode == 'agentic' or (obj.mode == 'auto' and len(obj.question) > 80) else 'basic'
        agent_state = {'retrieval_rounds': 1, 'tool_calls': 0, 'stop_reason': 'basic_retrieval'}
        if effective_mode == 'agentic':
            agent_state = agent_graph.invoke({'question': obj.question, 'retrieval_rounds': 0, 'tool_calls': 0, 'stop_reason': ''})
        sources = await QueryService.retrieve(db=db, user_id=user_id, is_admin=is_admin, obj=obj)
        if not sources:
            result = GetAnswerDetail(answer='当前知识库中没有足够资料回答该问题。', sources=[], requested_mode=obj.mode, effective_mode=effective_mode, status='insufficient_evidence', retrieval_rounds=agent_state['retrieval_rounds'], stop_reason='no_sources', trace_id=trace_id)
        else:
            ...
            result = GetAnswerDetail(answer=answer, sources=sources, requested_mode=obj.mode, effective_mode=effective_mode, status='answered', retrieval_rounds=agent_state['retrieval_rounds'], stop_reason=agent_state['stop_reason'], trace_id=trace_id)
        db.add(QueryLog(owner_id=user_id, knowledge_base_ids=obj.knowledge_base_ids, requested_mode=obj.mode, effective_mode=effective_mode, status=result.status, retrieval_rounds=result.retrieval_rounds, tool_calls=agent_state['tool_calls'], source_count=len(result.sources), latency_ms=int((time.perf_counter() - started) * 1000), trace_id=trace_id))
        return result
